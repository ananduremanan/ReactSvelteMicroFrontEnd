<script>
  ...
  export let enableExcelExport = false;
  export let excelName = "data";
  export let enablePdfExport = false;
  export let pdfName = "data";
  export let gridContainerClass = "";
  export let gridButtonClass =
    "px-1 py-2 bg-white border rounded-lg text-xs text-black";
  export let gridHeaderClass = "";
  export let gridGlobalSearchButtonClass = "";
  export let gridPaginationButtonClass = "";
  let currentPage = 0;
  let pageStart = 0;
  let pageEnd = 10;
  ...
  function handleSearch(searchParam2) {
    if (!lazy) {
        ...
        for (const key in item) {
          if (
            item[key]
              .toString()
              .toLowerCase()
              .includes(searchParam2.toLowerCase())
          ) {
            return true;
          }
        }
              ...
              <th class="px-1 py-3" colspan={columns.length}>
                <div class={twMerge("flex justify-end gap-2", gridHeaderClass)}>
                  ...
                  {#if enablePdfExport}
                    <button
                      on:click={() =>
                        exportToPDFHelper(workingDataSource, columns, pdfName)}
